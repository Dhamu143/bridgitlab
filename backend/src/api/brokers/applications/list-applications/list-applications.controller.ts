...
export class BrokerApplicationsListController {
  ...
  async post(
    ...
    const applicationData = plainToClass(ApplicationDto, body);
    const avgLoanAmount = await this.applicationEntity.getAverageLoanAmount()
    const loanAmount = applicationData.loanAmount !== avgLoanAmount ? body.loanAmount : null;
    const application = await this.applicationEntity.create({ ...applicationData, status: ApplicationStatus.Submitted, brokerId: user.id });
    return {
      success: true,
      loanAmount
    };
